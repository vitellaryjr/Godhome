...
function Stab:onStart()
    ...
    self.timer:everyInstant(1.2, function()
        self.timer:script(function(wait)
            ...
            local mace = self:spawnBullet("p4/failedchamp/mace", x, y, angle, arena.width)
            mace.alpha = 0
            self.timer:tween(0.1, mace, {alpha = 1})
            self.timer:tween(0.1, mace, {x = x - 20*dir}, "out-quad")
            wait(0.4)
            self.timer:tween(0.6, mace, {x = arena.x + (arena.width/2 + 30)*-dir}, "in-quad")
            wait(0.6)
            local s1 = self:spawnBulletTo(Game.battle.mask, "common/shockwave", arena.x - (arena.width/2)*-dir, y, 80, 80, 6)
            local s2 = self:spawnBulletTo(Game.battle.mask, "common/shockwave", arena.x - (arena.width/2)*-dir, y, 80, 80, -6)
